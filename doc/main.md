...
本次实验中，我实现的功能有：
- 读取`BMP`图片
- 采用`JPEG`压缩算法，将该`BMP`图片压缩为自定义格式文件
- 将压缩后的文件重新解码为`BMP`图片
- 输出`BMP`图片
### 2.3 开发环境及系统运行要求
- 开发环境
...
- 操作流程
    ...
    在根目录下执行`make`命令即可生成可执行文件，执行`./main`即可运行可执行文件，接下来根据提示操作即可，具体示例将在最后给出。
## 3. Technical Details
### 3.1 理论知识阐述
#### 3.1.1 无损压缩
- 基本原理
...
##### 3.1.1.3 差分编码（`DPCM`）
差分编码通过表示相邻数据之间的差，而不是直接表示数据本身，来减少数据冗余。在`JPEG`压缩过程中，`DC`分量的变化较小，通过DPCM能够显著提高压缩效率。
...
位图信息头(`BMPInfoHeader`):
- 主要关注的部分是`biWidth`(位图的宽)和`biHeight`(位图的高)和`biSizeImage`(位图的总大小)

位图数据：
- 为了简化处理，我没有使用调色盘，直接读入整个数据。
- 将连续的三个`unsigned char`分别作为一个像素的`B`, `G`, `R`分量

需要注意的是，由于后续需要对8*8的块进行`DCT`变换，且需要对`Cr, Cb`分量进行下采样，因此我们需要先将位图的宽高均填充为16的倍数，并重新设置位图信息头中的相关变量。
...
方便起见，我将4个色度样本取平均，得到了1个色度样本。易知，`Y`分量大小不变，`U`, `V`分量均缩小为原来的`1/4`
##### 2.2.2.3 分块`DCT`变换
...
##### 2.2.2.4 量化
参考`JPEG`标准亮度(`Luminance`)量化表和标准色度(`Chrominance`)量化表，根据输入的质量因子对量化表进行缩放后，再使用`DCT`变换后的矩阵除以缩放后的量化表后，四舍五入即可。

```cpp
    if (qf > 50) scaling_factor = (100 - qf) / 50;
    else scaling_factor = (50 / qf);
```
...
\end{bmatrix}\\[1ex]
\text{(a) Luminance quantization table}
\end{array}
\begin{array}{c}
\begin{bmatrix}
...
99 & 99 & 99 & 99 & 99 & 99 & 99 & 99 \\
\end{bmatrix}\\[1ex]
\text{(a) Chrominance quantization table}
\end{array}
$$

##### 2.2.2.5 `Zigzag`重构矩阵
为了将低频信息聚集在前面，高频信息聚集在后面。方便后续的RLE和DPCM编码进一步压缩数据，采用Zigzag扫描重构数据顺序。

具体实现中，虽然可以通过手动模拟坐标移动实现`Zigzag`重构矩阵，但是在我的代码中，计算了`Zigzag`重构后的元素在原矩阵中的坐标，从而简化了代码。
...
由于后续的熵编码部分需要知道具体每个值占多少个`bit`，我利用`(size, amplitude)`二元组来表示每个DC分量：`size`表示了`amplitude`需要多少`bit`来存储，`amplitude`则为具体的值(与前一个值的差值)

具体的 `size - amplitude` 对应关系如图：

![](img/size-amplitude.png)

观察可以发现，`amplitude`为正数时，实际上二进制首位均为`1`。因此我们可以将首位作为符号位，后续采用补码格式即可。如`010`代表`-6` `(size=3)`

##### 2.2.2.7 `AC`分量进行`RLE`
如原理部分所说，对于`AC`分量，我们可以将其转化为游程编码：每个编码是一个二元组`(skip, amplitude)`，`skip`表示跳过了多少个`0`，`amplitude`表示具体的值。

同理，后续的熵编码部分需要知道每个值占多少个`bit`，我利用`((skip << 4) + size, amplitude)`二元组来表示每个AC分量，`size`表示了当前`amplitude`需要多少`bit`来存储。

具体的 `size - amplitude` 对应关系与`DPCM`部分相同，在此不再赘述
...
接下来，根据范式`huffman`编码将二元组`(value, amplitude)`输出到压缩文件中。
...
##### 2.2.2.4 反Zigzag还原顺序
直接利用编码过程中计算得到的`Zigzag`坐标对应表，还原原本顺序即可。

##### 2.2.2.5 反量化
先将标准量化表根据质量因子缩放，再将`Zigzag`还原后的矩阵乘上对应的缩放后的量化表即可，具体的表内容与编码部分相同。
...
- 实验流程
    ...
    1. 读入`BMP`文件。将会输出该图像的宽和高
        ![](img/usage-1.png)
    2. 将`BMP`压缩为压缩文件。将会在量化部分输入质量因子，并输出一系列压缩过程，最后提示压缩成功
        ![](img/usage-2.png)
...
- 解码图 (qf = 10)
    ...
    ![](img/256256_qf10_decompression.bmp)

    大小为196,662字节

- 解码图 (qf = 20)

    ![](img/256256_qf20_decompression.bmp)

- 解码图 (qf = 40)

    ![](img/256256_qf40_decompression.bmp)

- 解码图 (qf = 50)

    ![](img/256256_qf50_decompression.bmp)

- 解码图 (qf = 100)

    ![](img/256256_qf100_decompression.bmp)


| qf | 压缩文件 | 压缩比 | 效果
| - | - | - | - |
| 10 | 2790 Bytes | 70:1 | 较差 |
| 20 | 4464 Bytes | 44:1 | 较好 |
| 40 | 7108 Bytes | 27.5:1 | 基本看不出区别 |
| 50 | 8056 Bytes | 24.5:1 | 基本看不出区别 |
| 100 | 35916 Bytes | 5.5:1 | 很差，满屏马赛克 |

奇怪的是，`qf=100`时，图片的最下面一小部分能正常解码，但我对比了编码前和解码后的YUV值，并没有找到很大的区别。希望在之后能修复这个问题。
